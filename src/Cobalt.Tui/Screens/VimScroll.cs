...
public static class VimScroll
{
    ...
    private static void ScrollListView(ListView view, AppCommand command, int? count)
    {
        ...
        var current = view.SelectedItem;
        var half = HalfPage(view);
        // From a null selection, mirror TG's own commands: a downward move lands on the first row
        // (seed -1 so +count reaches 0), an upward move lands on the last (seed total so -count
        // reaches total-1). A non-null selection uses its value unchanged (parity with the old path).
        var target = command switch
        {
            AppCommand.MoveDown => (current ?? -1) + (count ?? 1),
            AppCommand.MoveUp => (current ?? total) - (count ?? 1),
            AppCommand.HalfPageDown => (current ?? -1) + (half * (count ?? 1)),
            AppCommand.HalfPageUp => (current ?? total) - (half * (count ?? 1)),
