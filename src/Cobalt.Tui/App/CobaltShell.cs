...
public sealed class CobaltShell : Window
{
    ...
    // The preview pipeline (ADR 0024 / #49): owns the two-tier load, its workspace-lifetime
    // cancellation, and the list->pane wiring.
    private readonly PreviewCoordinator _previewCoord;
        ...
        TimeProvider? time = null)
    {
        ...
        _previewCoord = new PreviewCoordinator(
            ...
            () => _workItemList, () => _prList, _post, time);
        WireViewModel();
        WireKeys();
        WirePalette();
        WireTheme();
    ...
    private void ApplyWorkspaceLayout()
    {
        ...
        _previewCoord.LayoutChanged(Math.Max(1, panes.PreviewWidth - _previewPane.GetAdornmentsThickness().Horizontal - 1));
    ...
    internal int PreviewTextWidth => _previewCoord.TextWidth;
    ...
    private PrListView BuildPrList(PullRequestStoreAdapter pullRequests)
    {
        ...
        _prListVm = listVm;
        _previewCoord.Track(listVm); // see BuildWorkItemList
